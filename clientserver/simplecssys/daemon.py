...
#
# Modified for Python 3 (see also: http://web.archive.org/web/20131017130434/
...
class Daemon:
    ...
    def __init__(self, pid_file='_.pid', stdin='/dev/null', stdout='/dev/null', stderr='/dev/null'):
        self.stdin = stdin
        self.stdout = stdout
        self.stderr = stderr
    ...
    def daemonize(self):
        ...
        try:
            pid = os.fork()
            if pid > 0:
                # second parent exit (first child)
                sys.exit(0)
        except OSError as e:
